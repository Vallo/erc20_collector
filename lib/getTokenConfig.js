...
import { getUsdcBalance, collectUsdc, minimumUsdcWithdrawBalance, estimateUsdcGas } from './usdc/methods'
...
export function getConfig (token: Token): Config {
  if (token === TOKEN.USDT) {
    ...
    const config: Config = { getTokenBalance: getUsdcBalance, collectToken: collectUsdc, minWithdrawBalance: minimumUsdcWithdrawBalance, estimateGas: estimateUsdcGas }
    return config
  } else {
