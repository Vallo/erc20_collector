...
type Config = {
  ...
  estimateGas: (Web3, Wallet, string) => Number,
}

export function getConfig (token: Token): Config {
  if (token === TOKEN.USDT) {
    return { getTokenBalance: getUsdtBalance, collectToken: collectUsdt, minWithdrawBalance: minimumUsdtWithdrawBalance, estimateGas: estimateUsdtGas }
  } else if (token === TOKEN.DAI) {
    return { getTokenBalance: getDaiBalance, collectToken: collectDai, minWithdrawBalance: minimumDaiWithdrawBalance, estimateGas: estimateDaiGas }
  }
}
