...
export async function sendUsdt ({ web3, db }: Web3, from: Wallet, to: Address, amount: string): Promise<any> {
  const contract = new web3.eth.Contract(USDT_ABI, USDT_CONTRACT_ADDRESS)
...
export async function estimateUsdtGas ({ web3 }: Web3, to: Address, amount: string): Promise<string> {
  const contract = new web3.eth.Contract(USDT_ABI, USDT_CONTRACT_ADDRESS)
  return contract.methods.transfer(to, amount).estimateGas()
}

export async function collectUsdt ({ web3, db }: Web3, from: Wallet, amount: string): Promise<any> {
...
export function minimumUsdtWithdrawBalance () {
