...
getDb().then(async db => {
  ...
  wallets.forEach(async wallet => {
    ...
    DEBUG && console.log('USDT Balance: ' + wallet.address)
    ...
    if (!usdtBalanceBN.isZero()) {
      ...
      const estimatedGas = web3.utils.hexToNumber('0x80000') // await estimateGas({ web3 }, wallet.address, usdtBalance)
      ...
      if (gasBalanceBN.gt(gasRequired)) {
        ...
        return sendEth({ web3 }, gasProviderWallet, wallet.address, gasRequired)
      }
    } else {
