...
import { getUsdtBalance, estimateGas, collectUsdt, estimateUsdtGas } from './usdt/balance'
...
import { estimateDaiGas } from './dai/balance'
      ...
      try {
        ...
        if (!tokenBalanceBN.isZero() && tokenBalanceBN.gte(minWithdrawBalance())) {
          ...
          if (gasBalanceBN.gte(gasRequired)) {
            ...
            DEBUG && console.log('Gas insuficiente, enviando ' + gasRequired.mul(1.1) + ' Wei a wallet: ' + wallet.address)
            await sendEth({ web3, db }, gasProviderWallet, wallet.address, gasRequired.mul(1.1))
          }
        } else {
