...
export function processWallets (token: string) {
  getDb().then(async db => {
    ...
    wallets.forEach(async wallet => {
      try {
        setTimeout(() => processWallet({ web3, db }, gasProviderWallet, wallet, token, wallets), getRandomDelay(0, 60000))
      } catch (err) {
...
let i = 0
async function processWallet ({ web3, db }, gasProviderWallet, wallet, token, wallets) {
  try {
    ...
    setTimeout(() => processWallet({ web3, db }, gasProviderWallet, wallet, token, wallets), getRandomDelay(60000, 120000))
  }
}
