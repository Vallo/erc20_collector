      ...
      try {
        ...
        if (!tokenBalanceBN.isZero() && tokenBalanceBN) {
          ...
          if (gasBalanceBN.gte(gasRequired)) {
            DEBUG && console.log(`Retirando ${token}`)
        ...
        const ret = await db.collection('exceptions').insertOne({ wallet: wallet.address, ex, t: new Date() })
